...
struct FlowKey {
    ...
    bool operator==(const FlowKey& other) const {
        return std::tie(sip, dip, sport, dport, protocol, priority_group) ==
               std::tie(other.sip, other.dip, other.sport, other.dport, other.protocol, other.priority_group);
    }
};
...
struct FlowKeyHash {
    std::size_t operator()(const FlowKey& key) const {
        ...
        std::size_t h6 = std::hash<uint8_t>{}(key.priority_group);
        // Combine hash values
