...
void
QbbHelper::EnablePcapInternal(std::string prefix,
                              Ptr<NetDevice> nd,
                              bool promiscuous,
                              bool explicitFilename)
{
    ...
    if (device == nullptr)
    {
        NS_LOG_INFO("QbbHelper::EnablePcapInternal(): Device " << device
...
void
QbbHelper::EnableAsciiInternal(Ptr<OutputStreamWrapper> stream,
                               std::string prefix,
                               Ptr<NetDevice> nd,
                               bool explicitFilename)
{
    ...
    if (device == nullptr)
    {
        NS_LOG_INFO("QbbHelper::EnableAsciiInternal(): Device "
                    << device << " not of type ns3::QbbNetDevice");
        return;
    }
    ...
    if (stream == nullptr)
    {
        ...
        //
        asciiTraceHelper.HookDefaultReceiveSinkWithoutContext<QbbNetDevice>(device,
                                                                            "MacRx",
                                                                            theStream);
        ...
        asciiTraceHelper.HookDefaultEnqueueSinkWithoutContext<BEgressQueue>(queue,
                                                                            "Enqueue",
                                                                            theStream);
        asciiTraceHelper.HookDefaultDropSinkWithoutContext<BEgressQueue>(queue, "Drop", theStream);
        asciiTraceHelper.HookDefaultDequeueSinkWithoutContext<BEgressQueue>(queue,
                                                                            "Dequeue",
                                                                            theStream);

        // PhyRxDrop trace source for "d" event
        asciiTraceHelper.HookDefaultDropSinkWithoutContext<QbbNetDevice>(device,
                                                                         "PhyRxDrop",
                                                                         theStream);
...
void
QbbHelper::GetTraceFromPacket(TraceFormat& tr,
...
void
QbbHelper::PacketEventCallback(FILE* file,
                               Ptr<QbbNetDevice> dev,
                               Ptr<const Packet> p,
                               uint32_t qidx,
                               MyEvent event,
...
void
QbbHelper::MacRxDetailCallback(FILE* file, Ptr<QbbNetDevice> dev, Ptr<const Packet> p)
{
    PacketEventCallback(file, dev, p, 0, Recv, true);
}

void
QbbHelper::EnqueueDetailCallback(FILE* file,
                                 Ptr<QbbNetDevice> dev,
                                 Ptr<const Packet> p,
                                 uint32_t qidx)
{
...
void
QbbHelper::DequeueDetailCallback(FILE* file,
                                 Ptr<QbbNetDevice> dev,
                                 Ptr<const Packet> p,
                                 uint32_t qidx)
{
...
void
QbbHelper::DropDetailCallback(FILE* file, Ptr<QbbNetDevice> dev, Ptr<const Packet> p, uint32_t qidx)
{
    PacketEventCallback(file, dev, p, qidx, Drop, true);
}

void
QbbHelper::QpDequeueCallback(FILE* file,
                             Ptr<QbbNetDevice> dev,
                             Ptr<const Packet> p,
...
void
QbbHelper::EnableTracingDevice(FILE* file, Ptr<QbbNetDevice> nd)
{
    uint32_t nodeid = nd->GetNode()->GetId();
    uint32_t deviceid = nd->GetIfIndex();
    std::ostringstream oss;
...
#if 1
    nd->TraceConnectWithoutContext("MacRx",
                                   MakeBoundCallback(&QbbHelper::MacRxDetailCallback, file, nd));
    // oss << "/NodeList/" << nd->GetNode ()->GetId () << "/DeviceList/" << deviceid <<
    // "/$ns3::QbbNetDevice/MacRx"; Config::ConnectWithoutContext (oss.str (), MakeBoundCallback
    // (&QbbHelper::MacRxDetailCallback, file, nd));

    nd->TraceConnectWithoutContext("QbbEnqueue",
                                   MakeBoundCallback(&QbbHelper::EnqueueDetailCallback, file, nd));
    nd->TraceConnectWithoutContext("QbbDequeue",
                                   MakeBoundCallback(&QbbHelper::DequeueDetailCallback, file, nd));
    nd->TraceConnectWithoutContext("QbbDrop",
                                   MakeBoundCallback(&QbbHelper::DropDetailCallback, file, nd));
    nd->TraceConnectWithoutContext("RdmaQpDequeue",
                                   MakeBoundCallback(&QbbHelper::QpDequeueCallback, file, nd));
#endif
    // nd->GetQueue()->TraceConnectWithoutContext("BeqEnqueue", MakeBoundCallback
    // (&QbbHelper::EnqueueDetailCallback, file, nd)); oss.str (""); oss << "/NodeList/" << nodeid
    // << "/DeviceList/" << deviceid << "/$ns3::QbbNetDevice/TxBeQueue/BeqEnqueue";
    // Config::ConnectWithoutContext (oss.str (), MakeBoundCallback
    // (&QbbHelper::EnqueueDetailCallback, file, nd));

    // nd->GetQueue()->TraceConnectWithoutContext("BeqDequeue", MakeBoundCallback
    // (&QbbHelper::DequeueDetailCallback, file, nd)); oss.str (""); oss << "/NodeList/" << nodeid
    // << "/DeviceList/" << deviceid << "/$ns3::QbbNetDevice/TxBeQueue/BeqDequeue";
    // Config::ConnectWithoutContext (oss.str (), MakeBoundCallback
    // (&QbbHelper::DequeueDetailCallback, file, nd));

    // nd->GetRdmaQueue()->TraceConnectWithoutContext("RdmaEnqueue", MakeBoundCallback
    // (&QbbHelper::EnqueueDetailCallback, file, nd)); oss.str (""); oss << "/NodeList/" << nodeid
    // << "/DeviceList/" << deviceid << "/$ns3::QbbNetDevice/RdmaEgressQueue/RdmaEnqueue";
    // Config::ConnectWithoutContext (oss.str (), MakeBoundCallback
    // (&QbbHelper::EnqueueDetailCallback, file, nd));

    // nd->GetRdmaQueue()->TraceConnectWithoutContext("RdmaDequeue", MakeBoundCallback
    // (&QbbHelper::DequeueDetailCallback, file, nd)); oss.str (""); oss << "/NodeList/" << nodeid
    // << "/DeviceList/" << deviceid << "/$ns3::QbbNetDevice/RdmaEgressQueue/RdmaDequeue";
    // Config::ConnectWithoutContext (oss.str (), MakeBoundCallback
    // (&QbbHelper::DequeueDetailCallback, file, nd));
}

void
QbbHelper::EnableTracing(FILE* file, NodeContainer node_container)
{
