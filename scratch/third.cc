...
void
CheckNodeRoutingComponents(NodeContainer nodes)
{
    std::cout << "\n================== Node Routing Component Check ==================\n";
    ...
    for (uint32_t i = 0; i < nodes.GetN(); ++i)
    {
        ...
        if (ipv4)
        {
            std::cout << "  ✅ Ipv4 is installed.\n";
        }
        else
        {
            std::cout << "  ❌ Ipv4 is NOT installed.\n";
        }

        // Check for GlobalRouterInterface
        Ptr<GlobalRouter> gr = node->GetObject<GlobalRouter>();
        if (gr)
        {
            std::cout << "  ✅ GlobalRouterInterface is installed.\n";
        }
        else
        {
            std::cout << "  ❌ GlobalRouterInterface is NOT installed.\n";
        }

        // List NetDevices
        uint32_t nDevs = node->GetNDevices();
        std::cout << "  📦 Device count: " << nDevs << "\n";
        ...
        for (uint32_t j = 0; j < nDevs; ++j)
        {
            ...
            std::cout << "    🛠 Device " << j << ": " << type;

            if (ch)
            {
                std::cout << " | ✅ Connected to Channel ID: " << ch->GetId() << "\n";
            }
            else
            {
                std::cout << " | ❌ Not connected to any channel\n";
            }
        }
...
void
PrintAllChannelsAndDevices()
{
    std::cout << "\n================== Channel Connection Report ==================\n";

    uint32_t channelCount = ChannelList::GetNChannels();

    for (uint32_t i = 0; i < channelCount; ++i)
    {
        Ptr<Channel> ch = ChannelList::GetChannel(i);
        std::cout << "🔗 Channel ID: " << ch->GetId()
                  << " | Type: " << ch->GetInstanceTypeId().GetName()
                  << " | Devices: " << ch->GetNDevices() << "\n";
        ...
        for (uint32_t j = 0; j < ch->GetNDevices(); ++j)
        {
            ...
            if (dev == nullptr)
            {
                std::cout << "   ⚠️  Device " << j << " is nullptr! (Invalid NetDevice)\n";
                continue;
            }
            ...
            if (node == nullptr)
            {
                std::cout << "   ⚠️  Device " << j << " has no parent Node!\n";
                continue;
            }
            ...
            std::cout << "   📦 Device " << deviceIndex << " on Node " << node->GetId()
    ...
    std::cout << "================== End of Channel Report ======================\n";
}
...
main(int argc, char* argv[])
{
    ...
    endt = clock();
    std::cout << "Time taken: " << (double)(endt - begint) / CLOCKS_PER_SEC << " seconds\n";
}
